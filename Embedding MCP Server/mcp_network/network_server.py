"""SSE server wiring for embedding MCP."""
...
def create_sse_app(service, host: str = "127.0.0.1", port: int = 8000) -> FastMCP:
    app = FastMCP("embedding-mcp-network", host=host, port=port)

    @app.tool()
    async def embed_text(text: str) -> str:
        try:
            from embedding_mcp.mcp_local.handlers import handle_embed
